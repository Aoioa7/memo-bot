...
export async function POST(request: Request) {
	...
	if (e.message.text == "/manual") {
		client.replyMessage(token, {
			type: 'text',
			text: `
			現在のモードは${mode}(-1がお喋りモード、0がメモモード、1がメモ内容入力モード、2がリマインド設定モード)\n

			-1の時、
			『@memo-mode』でメモモードに移行(それ以外の入力はお喋りとみなされる)\n
			
			0の時、
			『@chat-mode』でお喋りモードに移行\n
			『/show』で現在のメモをリストアップ\n
			『/delete+空白+[メモのID(「/show」で見れる])』でメモを削除\n
			『/create+空白+[タイトル(20字以内)]』でメモを作成開始&1に移行\n

			1の時、
			送信したチャットはそのままメモの内容として登録される&2に移行\n
			
			２の時、
			１以上の数字を入力するとその日数が過ぎたときにメモについて通知される&0に移行\n
			ちなみに、[x]を入力すれば未設定でスキップできる\n
